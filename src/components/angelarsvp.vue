<template>
    <div class="container" role="main" aria-label="18th Birthday RSVP Invitation">
      ...
      <p class="invitation-text">
        Join us in celebrating the joy and happiness on ANGELA's special day!
        We cordially invite you to be a part of a celebration filled with laughter,
      ...
      <form @submit.prevent="handleSubmit" aria-label="RSVP Form">
        ...
        <div class="radio-group" role="radiogroup" aria-labelledby="attendanceLabel">
          <label>
            <input
              @click="handleAttendanceChange('Yes')"
              type="radio"
              v-model="attendance"
              value="Yes"
              required
          ...
          <label>
            <input
              @click="handleAttendanceChange('No')"
              type="radio"
              v-model="attendance"
              value="No"
            />
        ...
        <p class="invitation-text">
          Bring your enthusiasm and festive spirit! We hope you can make it and celebrate with us.
        </p>
  ...
  function findPerson() {
    ...
    const person = eightCandle.find((r) =>
      r.value.toLowerCase().includes(inputNameLower)
    )
  ...
  function handleSubmit() {
    if (!name.value.trim() || !attendance.value) {
      alert('Please enter your name and select attendance.')
      return
    }
  
    thankYouMessage.value = `Thank you, ${name.value}! Your RSVP that you will ${
      attendance.value === 'Yes' ? 'attend' : 'not attend'
    } has been received.`
